...
export interface Database {
  public: {
    Tables: {
      ...
      product_variants: { Row: { id:string; product_id:string; name:string; approximate_weight_grams:number|null; price_cents:number|null; vat_rate:number; status:"draft"|"active"|"unavailable"|"discontinued"; display_order:number; stock_tracking:boolean; stock_quantity:number; subscribable:boolean; created_at:string; updated_at:string }; Insert: { id?:string; product_id:string; name:string; approximate_weight_grams?:number|null; price_cents?:number|null; vat_rate:number; status?:"draft"|"active"|"unavailable"|"discontinued"; display_order?:number; stock_tracking?:boolean; subscribable?:boolean }; Update: Partial<Database["public"]["Tables"]["product_variants"]["Insert"]>; Relationships: [] };
    ...
    Functions: {
      ...
      register_stock_movement: { Args: { p_product_variant_id: string; p_type: "entrada" | "merma" | "ajuste"; p_quantity: number; p_notes?: string | null }; Returns: { id: string; product_variant_id: string; type: string; quantity: number; order_id: string | null; notes: string | null; created_by: string | null; created_at: string } };
