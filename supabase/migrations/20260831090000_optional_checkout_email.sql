-- Reduce fricción en el checkout: el email deja de ser obligatorio. Se
-- valida solo si se proporciona (tiene que parecer un email de verdad);
-- vacío o null se guarda como null. Sin email, el pedido no recibe
-- confirmación ni recordatorio de recogida por correo (los triggers de
-- notificaciones ya usan coalesce(customer_email,guest_email) y toleran
-- null), pero el nombre y el teléfono siguen siendo obligatorios.
